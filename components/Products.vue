...
onMounted(async () => {
  ...
  watch(filterController, (newValue, oldValue) => {
    if (filterController.value == "1") {
      sortedProducts.value = [...store.products].sort(compareByPrice);
    } else if (filterController.value == "2") {
      sortedProducts.value = [...store.products].sort(compareByStock);
    } else if (filterController.value == "3") {
      sortedProducts.value = [...store.products].sort(compareByName);
    }
  ...
  <div class="wrapper">
    <div class="spinnnnner mt-5 justify-content-center w-100 h-100 d-flex">
      <div
        ...
        role="status"
        v-show="!store.products"
      >
        <span class="visually-hidden">Loading...</span>
      </div>
    </div>
    <div class="" v-show="store.products">
      <select
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center pt-4 mx-auto">
          ...
          <li
            class="page-item"
            v-for="page in Math.ceil(sortedProducts.length / itemsPerPage)"
            :key="page"
          >
